...
- `Launch/` (Application layer; named `Launch` to avoid clashing with `System.Windows.Application` in WPF): `GameLauncher.ConnectAsync(ServerProfile)` builds `FiveMLaunchOptions` and returns a `LaunchResult` — `Connect(Uri)` after delegating the `fivem://connect/...` URI to `IGameProcessLauncher` (the process seam), or `OpenClient(GameClient)` for `FiveMEnhanced` (no launch, UI opens the client later). Real process start is out of scope for now.
