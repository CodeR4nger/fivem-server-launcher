...
Before handing the connect URI to the process launcher, the launcher primes the Legacy FiveM client's `CitizenFX.ini` (`%localappdata%\FiveM\FiveM.app\CitizenFX.ini`, `[Game]` section) so the client boots already in the state the server requires — no restart-on-connect, no executable surprise. This includes **clearing** a stale `PoolSizesIncrease`: when the server runs default pool sizes (no `sv_poolSizesIncrease`), the launcher resets the key to an empty value so a previously primed increase from another server doesn't linger.
...
- `[Game] PoolSizesIncrease = <json>` — the server's `sv_poolSizesIncrease` value verbatim (e.g. `{"CWeaponComponentInfo":500}`), passed through unchanged. Writing it before launch prevents the client's restart-on-mismatch. When the server publishes none (default pool sizes), the key is reset to an empty value (`PoolSizesIncrease=`) so stale increases from previous servers are cleared.
...
The write is best-effort and gated: it only happens for CFX-validated Legacy FiveM profiles (those always have a known pool-size state to prime), only when the current `.ini` value differs, and never changes any other key or section. A failed or skipped write never fails the connect — the client falls back to its built-in self-correction.
...
6. As a player, I want pool sizes reset to defaults when I join a server that runs default pool sizes, so that a raise primed for a previous server doesn't linger and trigger a restart-on-connect.
...
- **New seam `Launch/ICitizenFxPreparer`** (+ real `Launch/CitizenFxPreparer`): `Task PrimeAsync(ServerProfile profile)`. Sole owner of the decision:
  ...
  - Build target values: `DefaultBuild` ← `Requirements.DefaultBuild` when `> 0`; else if `Requirements.ReplaceExecutable == true` and `Requirements.GameBuild` has a value ← `Requirements.GameBuild`; `PoolSizesIncrease` ← `Requirements.PoolSizesIncrease` verbatim when non-blank, else `""` (reset to defaults — the key is always primed for a CFX-validated Legacy FiveM profile, since its pool-size state is always known).
...
- `CitizenFxPreparer` is tested with fake `IClientInstallLocator` + fake `ICitizenFxConfigWriter`: gates (unvalidated/Enhanced/RedM/none-installed → no write), source precedence (`sv_defaultGameBuild` wins over legacy fallback, `0` falls through), `sv_poolSizesIncrease` passed through verbatim, default/blank pool sizes reset to an empty value, best-effort swallow of writer/locator failures.
- `CitizenFxConfigWriter` is tested against the real file layout with temp files/unique paths per test (prior art: `FileSettingsStorageTests` + `TempSettingsDirectory`): create-when-absent, update-only-when-changed (uphold the no-touch-unless-different contract, incl. an already-empty value being a no-op), preserve all other keys/sections verbatim, never touch keys outside the target set.
...
- Naming `<Method>_Should<Expectation>` with Given/When/Then; full suite `dotnet test` green (155 total).
