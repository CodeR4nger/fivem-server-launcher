...
- **MainViewModel wiring**: the connect flow (a) resolves the address (typed or from the selected saved server), (b) applies the merged manual+published requirements to the profile, (c) checks readiness of the required apps, (d) includes the readiness outcome in `StatusText` (e.g. "Steam ✓ / Discord ✓" when required+ready; "Requires Steam (not running)" when required+missing), and (e) launches as today. No business logic in code-behind.
